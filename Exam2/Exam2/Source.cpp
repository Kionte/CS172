...
int** fillArray(unsigned int numRows, unsigned int numCols)
{
	...
	My2DArray = new int*[numRows];	// have the double pointer point to the single pointer and have the single pointer be the size of the number of rows
	//<=== PUT YOUR CODE HERE!
	...
	for (int i = 0; i < numRows; i++) {
		...
		My2DArray[i] = new int[numCols]; //have the single pointer at each location (rows) point to an array (col) 						
		//<=== PUT YOUR CODE HERE!
...
void deleteArray(int** A, int size)
{
	...
	//YOUR CODE GOES HERE
	for (int i = 0; i < size; i++) { // loop used to ncrement through each row 
		delete[] A[i]; // used to delete the each row 
	}
	delete A; // deletes the entire thing! 
